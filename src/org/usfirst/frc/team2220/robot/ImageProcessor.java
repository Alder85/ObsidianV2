...
public class ImageProcessor {
	...
	// Constants
	NIVision.Range TOTE_HUE_RANGE = new NIVision.Range(90, 135); //bright green/white
	NIVision.Range TOTE_SAT_RANGE = new NIVision.Range(80, 255); 
	NIVision.Range TOTE_VAL_RANGE = new NIVision.Range(0, 255);
	double AREA_MINIMUM = 0.50; // Default Area minimum for particle as a percentage of total image area
	...
	public ImageProcessor(int inSession) {
		...
		criteria[0] = new NIVision.ParticleFilterCriteria2(NIVision.MeasurementType.MT_AREA_BY_IMAGE_AREA, AREA_MINIMUM, AREA_MAXIMUM, 0, 0);
	...
	boolean lookForTarget()
	{
		...
		if(numParticles > 0)
		{
			...
			for(int particleIndex = 0; particleIndex < numParticles; particleIndex++)
			{
				...
				//if(par.BoundingRectLeft > 100) //use this to limit frame search size
					particles.add(par);
			}
			particles.sort(null);
			...
			for(int i = 0;i < particles.size();i++)
			{
				...
				int height = (int)particles.elementAt(i).BoundingRectBottom - (int)particles.elementAt(i).BoundingRectTop;
				int width = (int)particles.elementAt(i).BoundingRectRight - (int)particles.elementAt(i).BoundingRectLeft;
